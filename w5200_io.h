...
#define __SOCKET_REGISTER8(name, address)\
  static inline void write_##name(uint16_t _s, uint8_t _data) {\
    wiznet_w_reg((W52_SOCK_OFFSET * _s) + address, _data);\
  }\
  static inline uint8_t read_##name(uint16_t _s) {\
    return wiznet_r_reg((W52_SOCK_OFFSET * _s) + address);\
  }
...
#define __SOCKET_REGISTER_N(name, address, size)\
  ...
  __GP_REGISTER8 (IR2,    W52_IR2);    // Interrupt
  __GP_REGISTER8 (PHYST,  W52_PHYSTATUS);    // Interrupt
  __GP_REGISTER8 (IMR2,   W52_IMR2);    // Interrupt MaskW52_IMR2
  __GP_REGISTER8 (IMR,    W52_IMR);    // Interrupt MaskW52_IMR2
  ...
  __SOCKET_REGISTER8(SnIRM,       W52_SOCK_IMR)        // Interrupt
  ...
  __SOCKET_REGISTER8(SnTX_MEM,    W52_SOCK_TXMEM_SIZE)
  __SOCKET_REGISTER8(SnRX_MEM,    W52_SOCK_RXMEM_SIZE)
